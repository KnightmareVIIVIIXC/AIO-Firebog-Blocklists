...
def find_blocking_blocklists(
    target_domain, green_sources, blue_sources, bog_sources, search_sources
):
    ...
    for name, source in green_sources.items():
        if search_sources in (
            ...
            "all",
            "all sources",
        ) and check_domain_in_blocklist(source, target_domain):
    ...
    if (
        search_sources in ("g", "green", "green sources", "a", "all", "all sources")
        and blocking_green_blocklists
    ):
        print(
            Fore.GREEN
            + f"Domain '{target_domain}' found in the following green sources:\n"
        )
        for blocklist in blocking_green_blocklists:
            print(Fore.GREEN + f" - {blocklist}\n")

    if (
        search_sources in ("b", "blue", "blue sources", "a", "all", "all sources")
        and blocking_blue_blocklists
    ):
        print(
            Fore.BLUE
            + f"Domain '{target_domain}' found in the following blue sources:\n"
        )
        for blocklist in blocking_blue_blocklists:
            print(Fore.BLUE + f" - {blocklist}\n")

    if (
        search_sources in ("o", "bog", "bog only", "a", "all", "all sources")
        and blocking_bog_blocklists
    ):
        print(
            Fore.CYAN
            + f"Domain '{target_domain}' found in the following bog only sources:\n"
        )
        for blocklist in blocking_bog_blocklists:
            print(Fore.CYAN + f" - {blocklist}\n")

    if (
        not blocking_green_blocklists
        and not blocking_blue_blocklists
        and not blocking_bog_blocklists
    ):
...
if __name__ == "__main__":
    ...
    bog_blocklist_sources = {
        ...
        "Disconnect-Me Ad (Ads)": "https://s3.amazonaws.com/lists.disconnect.me/simple_ad.txt",
    ...
    while True:
        ...
        while True:
            search_option = input(
                "Search in (g)reen, (b)lue, b(o)g, (a)ll sources, or (e)xit: "
            ).lower()
